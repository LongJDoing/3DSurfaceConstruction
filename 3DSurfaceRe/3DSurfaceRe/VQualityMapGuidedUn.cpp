...
CVQualityMapGuidedUn::CVQualityMapGuidedUn(void)
{
}
...
void CVQualityMapGuidedUn::PushQueue(Queue *queue, int data)
{
	QNode *p = NULL;
...
int CVQualityMapGuidedUn::PopQueue(Queue *queue)
{
	QNode *p = NULL;
	int data;
	if(queue->first == NULL)
...
Note: 垂直光栅的线扫描算法，其展开的方向为X轴方向
  --------------------------------------------------------------*/
double CVQualityMapGuidedUn::LineScan(double CurrentValue, double NeighborValue,double UnwrapNeighborValue, CPoint CurrentPoint, CPoint NeighborPoint)
{
	double temp = 0.0;
	temp = CurrentValue - NeighborValue;
	//if(abs(temp) < 1.0e-5)
	//{
	//	if((CurrentPoint.y - NeighborPoint.y) == 0)
	//	{
	//		if((CurrentPoint.x - NeighborPoint.x) == 1)
	//			CurrentValue = UnwrapNeighborValue + PI/ 20; // 40个pixel一个周期，一个周期长为PI
	//		else
	//			CurrentValue = UnwrapNeighborValue - PI/ 20;
	//	}
	//	else
...
Note: 
  --------------------------------------------------------------*/
double * CVQualityMapGuidedUn::UnwrapPhaseMap(double * phasemap, int * labelmap, int width, int height,CPoint startpoint, CPoint refpoint)
{
	Queue *queue = NULL;
//	queue = (Queue*)malloc(sizeof(Queue));
	...
	while(index < 4)
	{
		...
		//for(int cy = startpoint.y; cy > startpoint.y - 8; cy--)
		//	for(int cx = startpoint.x; cx < startpoint.x + 4; cx++)
			{
				CurrentPoint.x = cx;
				CurrentPoint.y = cy;
...
Note: 计算质量图2掩模区域的满足条件的初始点作为相位展开的初始点
  --------------------------------------------------------------*/
CPoint CVQualityMapGuidedUn::FindStartPosition(double * phasemap, int *labelmap, CPoint centerpoint, int width, int height)
{
	CPoint StartPoint ;
