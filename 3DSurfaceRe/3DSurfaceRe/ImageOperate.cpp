...
void  CImageOperate::calotsuTwoThreshold(int * m_hisArr, int &T1, int &T2)
{
	double MaxValue = 0;
	int otsuthreshold1, otsuthreshold2;
	double w = 0;
    double u = 0;
    for(int i = 0; i < 256; i++)
    {
	...
	for(int t1 = 0; t1 < 256-1; t1++)
		for(int t2 = t1+1; t2 < 256; t2++)
		{
			...
			for(int i = t2+1; i < 256; i++)
				u2 += i*m_hisArr[i]/w2;
			double tempValue;
			tempValue = w0*(u-u0)*(u-u0) + w1*(u-u1)*(u-u1) + w2*(u-u2)*(u-u2);
...
void CImageOperate::calconvolution(double ** image,  int conlength, double *m_conArr, int width, int height)
{
	double * color = new double[conlength];
	double r;
	 if(conlength == 4)
    {
